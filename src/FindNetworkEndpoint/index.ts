...
export class FindNetworkEndpoint {
  ...
  find(): number {
    ...
    if (this.network[this.startNodeId] === undefined) {
      ...
      const networkArrayObject = Object.entries(this.network).splice(
        Object.keys(this.network).indexOf(this.startNodeId.toString())
      );

      for (const [nodeId, nodeValue] of networkArrayObject) {
